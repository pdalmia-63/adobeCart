...
for (let i = 0; i < data.length; i++) {
  ...
  const itemImage = items.image;
  const itemActualPrice = items.price.actual;
  const itemDisplayPrice = items.price.display;
  const itemDiscount = items.discount;

  const itemInnerHTML = '<itmes>' +
    '<thumbnail-image>' +
      '<button class="discount">' + itemDiscount + '% off</button>' +
      '<img src="' + itemImage + '">' +
    '</thumbnail-image>' +
    '<div class="desc">' +
      '<p>' + itemName + '</p>' +
      '<price>' +
        '<p><strike>$' + itemDisplayPrice + '</strike> $' + itemActualPrice + '</p>' +
        '<p class="btn-add">Add to Cart</p>' +
      '</price>' + 
    '</div>' +
    '</itmes>';
  itemsHTML = itemsHTML + itemInnerHTML;
}
